...
const SCRIPT_URL_MONITOR_ALERTAS =
  "https://script.google.com/macros/s/AKfycbyX9HH80G0wwR14PfcYiT1Qink6LujZO2vz0nTbBLABT2LyGrRspCI6rakRJ1acULfv/exec"; //

// Elementos del DOM para el modal de detalle
let alertaModalElement,
  modalDetalleTituloElement,
  modalDetalleCuerpoElement,
  modalDetalleCerrarBtnElement;
...
let listaAlertasContentElement,
  loadingAlertasElement,
  noAlertasElement,
  errorAlertasElement;

// Elementos del DOM para la tabla de historial
let cuerpoTablaHistorialElement,
  loadingHistorialRowElement,
  noHistorialRowElement,
  errorHistorialRowElement;
...
const ALERTAS_POR_PAGINA = 50;
let paginaActualHistorial = 1;
let totalPaginasHistorial = 1;
let historialCompleto = [];
let paginacionHistorialContainer = null;
...
function _addManagedEventListener(element, type, handler, options = false) {
  if (element) {
    ...
    console.warn(
      `ALERTAS: Intento de añadir listener a elemento nulo (tipo: ${type})`
    );
  }
}
...
async function _appsScriptRequest(action, params = {}, method = "GET") {
  console.log(
    "ALERTAS: Preparando solicitud a Apps Script:",
    action,
    params,
    method
  );
  ...
  if (method.toUpperCase() === "GET") {
    ...
    return fetch(fullUrl, {
      ...
      .then((response) => {
        console.log(
          `ALERTAS: Respuesta recibida, status:`,
          response.status,
          response.statusText
        );
        if (!response.ok) {
          console.error(
            `ALERTAS: Error HTTP ${response.status}: ${response.statusText}`
          );
          throw new Error(
            `ALERTAS: Error HTTP ${response.status}: ${response.statusText}`
          );
        ...
        return response.text().then((text) => {
          console.log(
            `ALERTAS: Texto de respuesta (primeros 100 chars):`,
      ...
      .then((text) => {
        try {
          ...
          try {
            ...
            console.log(
              `ALERTAS: Datos parseados como JSON correctamente:`,
              data
            );
          } catch (e) {
            console.log(
              `ALERTAS: El texto no es JSON válido, usándolo como está`
            );
            return {
              success: true,
              data: text,
              mensaje: "Respuesta en texto plano",
            };
          }
          ...
          if (data && data.success === false) {
            console.error(
              `ALERTAS: Error Apps Script (GET "${action}"):`,
            ...
            console.log(
              `ALERTAS: Respuesta GET para "${action}" exitosa:`,
              data
            );
            return data;
          }
          ...
          console.error(
            `ALERTAS: Error al procesar respuesta GET para "${action}":`,
            parseError
          );
          console.error(`ALERTAS: Texto de respuesta problematico:`, text);
          throw new Error(
            "Error al procesar respuesta del servidor: " + parseError.message
          );
        }
      })
      .catch((error) => {
    ...
    console.log(
      `ALERTAS: Enviando POST. Acción: ${action}, Cuerpo (parcial):`,
      JSON.stringify(postBody).substring(0, 200) + "..."
    );
    return fetch(SCRIPT_URL_MONITOR_ALERTAS, {
          ...
          return response
            ...
            .then((errData) => {
              throw new Error(
                errData.error ||
                  errData.message ||
                  `ALERTAS: Error HTTP ${response.status}`
              );
            });
        }
      ...
      .then((data) => {
        if (data.success === false) {
          console.error(
            `ALERTAS: Operación POST "${action}" falló en servidor:`,
            data.error || data.message
          );
          throw new Error(
            data.error || data.message || "La operación POST de alertas falló."
          );
        }
...
function inicializarModulo() {
  ...
  modalDetalleCerrarBtnElement = document.getElementById(
    "modalDetalleCerrarBtn"
  ); //
  modalDetalleDatosParseadosDiv = document.getElementById(
    "modalDetalleDatosParseados"
  ); //
  ...
  cuerpoTablaHistorialElement = document.getElementById(
    "cuerpo-tabla-historial"
  ); //
  ...
  if (
    !listaAlertasContentElement ||
    !cuerpoTablaHistorialElement ||
    !alertaModalElement
  ) {
    console.error(
      "ALERTAS: Faltan elementos cruciales del DOM. La sección no puede inicializarse correctamente."
    );
...
function _inicializarModalDetalle() {
  ...
  if (!alertaModalElement || !modalDetalleCerrarBtnElement) {
    console.error(
      "ALERTAS: Elementos del modal de detalle no están disponibles para inicializar listeners."
    );
    return;
  }
  _addManagedEventListener(
    modalDetalleCerrarBtnElement,
    "click",
    _cerrarModalDetalle
  );
...
function _escKeyHandlerModal(event) {
  if (
    event.key === "Escape" &&
    alertaModalElement &&
    !alertaModalElement.classList.contains("hidden")
  ) {
    //
    _cerrarModalDetalle();
  }
...
function _abrirModalDetalle(
  ...
  contenidoHtmlProcesado = contenidoHtmlProcesado
    .replace(/\n\s*\n{2,}/g, "\n\n")
    .trim();
...
function _crearElementoAlerta(alerta, headers, headerMap) {
  ...
  if (!uid) {
    console.warn(
      "ALERTAS: Alerta sin UID, no se puede crear elemento:",
      alerta
    );
    return null;
  }
...
function _renderizarPaginacionHistorial() {
  if (!paginacionHistorialContainer) {
    paginacionHistorialContainer = document.getElementById(
      "historial-paginado-container"
    );
    if (!paginacionHistorialContainer) return;
  }

  if (totalPaginasHistorial <= 1) {
    paginacionHistorialContainer.innerHTML = "";
    return;
  }

  let html = '<div class="paginacion-wrapper">';

  html += `<button class="btn-paginacion" ${paginaActualHistorial === 1 ? "disabled" : ""} onclick="window.alertas_cambiarPaginaHistorial(${paginaActualHistorial - 1})">
            ← Anterior
        </button>`;

  html += `<span class="paginacion-info">Página ${paginaActualHistorial} de ${totalPaginasHistorial}</span>`;

  html += `<button class="btn-paginacion" ${paginaActualHistorial === totalPaginasHistorial ? "disabled" : ""} onclick="window.alertas_cambiarPaginaHistorial(${paginaActualHistorial + 1})">
            Siguiente →
        </button>`;

  html += "</div>";
  paginacionHistorialContainer.innerHTML = html;
}

function _cambiarPaginaHistorial(nuevaPagina) {
  if (nuevaPagina < 1 || nuevaPagina > totalPaginasHistorial) return;
  paginaActualHistorial = nuevaPagina;
  _renderizarHistorialPaginado();
}

function _renderizarHistorialPaginado() {
  if (!cuerpoTablaHistorialElement || !Array.isArray(historialCompleto)) return;

  const inicio = (paginaActualHistorial - 1) * ALERTAS_POR_PAGINA;
  const fin = Math.min(inicio + ALERTAS_POR_PAGINA, historialCompleto.length);
  const historialPagina = historialCompleto.slice(inicio, fin);

  // Limpiar tabla
  const filas = Array.from(cuerpoTablaHistorialElement.querySelectorAll("tr"));
  filas.forEach((row) => {
    if (
      row.id !== "loading-historial-row" &&
      row.id !== "no-historial-row" &&
      row.id !== "error-historial-row"
    ) {
      row.remove();
    }
  });
  ...
  historialPagina.forEach((item, relIndex) => {
    const globalIndex = inicio + relIndex;
    try {
      const fila = cuerpoTablaHistorialElement.insertRow();

      const estadoVal = item.estado || "N/A";
      const revisadoVal = item.revisado || "N/A";
      const timestampVal = item.timestamp || "";
      const asuntoVal = item.asunto || "";
      const detallesVal = item.detalles || "No hay detalles disponibles.";

      const estadoHistClase =
        String(estadoVal)
          .toLowerCase()
          .replace(/[^a-z0-9-_]/g, "") || "desconocido";
      const revisadoHistClase =
        String(revisadoVal).toLowerCase() === "sí" ? "si" : "no";

      const estadoTitle =
        estadoVal === "Positivo"
          ? "La alerta coincidió con una regla y requiere atención."
          : "El email fue procesado pero no cumplió las condiciones.";

      fila.insertCell().innerHTML = `<div class="col-index">${globalIndex + 1}</div>`;
  ...
  _renderizarPaginacionHistorial();
}

window.alertas_cambiarPaginaHistorial = _cambiarPaginaHistorial;

async function _cargarYFiltrarAlertas() {
  console.log(
    "ALERTAS: _cargarYFiltrarAlertas iniciado. Actualizando referencias al DOM..."
  );
  ...
  if (!listaAlertasContentElement || !cuerpoTablaHistorialElement) {
    console.error(
      "ALERTAS: CRÍTICO - No se encuentran los contenedores principales en el DOM actual."
    );
    return;
  }
  ...
  if (cuerpoTablaHistorialElement) {
    ...
    const filas = Array.from(
      cuerpoTablaHistorialElement.querySelectorAll("tr")
    );
    filas.forEach((row) => {
      if (
        row.id !== "loading-historial-row" &&
  ...
  if (listaAlertasContentElement) {
    ...
    const itemsPrevios =
      listaAlertasContentElement.querySelectorAll(".alerta-item");
  ...
  try {
    ...
    console.log(
      "ALERTAS: Datos recibidos en _cargarYFiltrarAlertas:",
      response
    );
    ...
    if (loadingAlertasElement) {
      ...
      console.error(
        "ALERTAS: No se encontró loadingAlertasElement para ocultarlo."
      );
    }
    ...
    if (loadingHistorialRowElement) {
      ...
      console.error(
        "ALERTAS: No se encontró loadingHistorialRowElement para ocultarlo."
      );
    }
    ...
    if (
      ...
      const alertasPositivasNoRevisadas = todasLasAlertas
        .filter((alerta) => {
          const estadoActual = alerta[ESTADO_KEY]
            ? String(alerta[ESTADO_KEY]).toLowerCase()
            : "";
          const revisadoActual = alerta[REVISADO_KEY]
            ? String(alerta[REVISADO_KEY]).toLowerCase()
            : "";
          return estadoActual === "positivo" && revisadoActual !== "sí";
        })
        .sort(
          (a, b) =>
            (new Date(b[TIMESTAMP_KEY]) || 0) -
            (new Date(a[TIMESTAMP_KEY]) || 0)
        );

      console.log(
        `ALERTAS: ${alertasPositivasNoRevisadas.length} alertas positivas no revisadas encontradas.`
      );

      if (alertasPositivasNoRevisadas.length > 0) {
        console.log(
          `ALERTAS: Intentando renderizar ${alertasPositivasNoRevisadas.length} alertas.`
        );
        alertasPositivasNoRevisadas.forEach((alertaF, index) => {
          console.log(
            `ALERTAS: Creando elemento para alerta ${index + 1}/${alertasPositivasNoRevisadas.length} (UID: ${alertaF.UID || alertaF.uid})`
          );
          ...
          if (el) {
            if (listaAlertasContentElement) {
              ...
              console.log(
                `ALERTAS: Elemento añadido al DOM para alerta ${index + 1}`
              );
            } else {
              console.error(
                `ALERTAS: CRÍTICO - listaAlertasContentElement es NULL al intentar añadir alerta ${index + 1}`
              );
            }
          } else {
            console.warn(
              `ALERTAS: _crearElementoAlerta devolvió NULL para alerta ${index + 1}`
            );
          }
        });
      } else {
      ...
      // Preparar historial CON PAGINACIÓN
      console.log("ALERTAS: Iniciando renderizado de historial...");
      historialCompleto = [...todasLasAlertas]
        .sort(
          (a, b) =>
            (new Date(b[TIMESTAMP_KEY]) || 0) -
            (new Date(a[TIMESTAMP_KEY]) || 0)
        ...
        .map((item) => ({
          estado: item[ESTADO_KEY] || "N/A",
          revisado: item[REVISADO_KEY] || "N/A",
          timestamp: item[TIMESTAMP_KEY] || "",
          asunto: item["Asunto"] || item[headerMap["asunto"]] || "",
          detalles:
            item["Detalles_Disparo"] ||
            item[headerMap["detalles_disparo"]] ||
            "No hay detalles disponibles.",
        }));

      totalPaginasHistorial =
        Math.ceil(historialCompleto.length / ALERTAS_POR_PAGINA) || 1;
      paginaActualHistorial = 1;

      if (historialCompleto.length > 0) {
        console.log(
          `ALERTAS: Preparados ${historialCompleto.length} registros de historial para paginación.`
        );
        _renderizarHistorialPaginado();
      } else {
        console.log("ALERTAS: No hay historial para mostrar.");
        if (noHistorialRowElement)
          noHistorialRowElement.classList.remove("hidden");
      }
    } else {
      const errorMsg =
        (response && response.error) ||
        "Formato de datos incorrecto o fallo del servidor.";
...
function estadoResumenAlertas(alertas) {
  ...
  return alertas
    .map(
      (a) =>
        `${a.UID || a.uid || ""}-${(a.Revisado || a.revisado || "").toLowerCase()}`
    )
    .join(";");
}
  ...
  intervaloVerificacionCambios = setInterval(async () => {
    try {
      ...
      if (ultimoResumenAlertas === null) {
        ...
        console.log(
          "🔁 Cambio detectado en alertas (cantidad o revisados). Refrescando..."
        );
        ultimoResumenAlertas = resumenActual;
